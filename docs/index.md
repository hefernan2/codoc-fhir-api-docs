...
=== "JavaScript"
    ...
    ```javascript
    const response = await fetch('{API_URL}/v4.3.0/patient/', {
      method: 'POST',
...
!!! info "Important"
    The `id` field (`123`) is generated by the server. Use this ID to retrieve or reference the patient in subsequent requests.
...
- **`use: "official"`** - Primary key in database (use this for GET, UPDATE, DELETE operations)
...
In this example:
- Use `id: "123"` or `identifier.official: "123"` for API operations (GET, UPDATE, DELETE)
